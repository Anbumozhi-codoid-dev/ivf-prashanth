...
class CancelLeaveEvent extends LeaveEvent{
  ...
  CancelLeaveEvent(this.data, this.token,this.context);
}
...
class CancelLeaveLoadingState extends LeaveState {}

class CancelLeaveSuccessState extends LeaveState {
  var data;

  CancelLeaveSuccessState(this.data);
}

class CancelLeaveErrorState extends LeaveState {
  String error;
...
class LeaveBloc extends Bloc<LeaveEvent, LeaveState> {
  ...
  Stream<LeaveState> mapEventToState(LeaveEvent event) async* {
    ...
    if (event is CancelLeaveEvent) {
      yield await _cancelLeave(event);
    }
  ...
  // Future<LeaveState> _cancelLeave(CancelLeaveEvent event) async {
  //   var state;
  //   await APIManager().cancelLeave(
  //       payload: event.data,
  //       successBlock: (data) {
  //         state = CancelLeaveLoadingState();
  //         state = CancelLeaveSuccessState(data);
  //       },
  //       failureBlock: (exception) {
  //         state = CancelLeaveErrorState(exception.toString());
  //
  //         AppAlertController().showAlert(
  //             message: exception.toString(), inContext: event.context!);
  //       });
  ...
  Future<LeaveState> _cancelLeave(CancelLeaveEvent event) async {
    ...
    try{
     ...
     // print(formData.);
      ...
      if (response.statusCode == 200) {
        state=CancelLeaveSuccessState(response.data);
      ...
      if(error is DioException){
        ...
        AppAlertController().showAlert(message: error.response!.data['status']['message'], inContext: event.context!);
        state=CancelLeaveErrorState(error.response!.data['status']['message']);
      }else{
        AppAlertController().showAlert(message: error.toString(), inContext: event.context!);
        state=CancelLeaveErrorState(error.toString());
